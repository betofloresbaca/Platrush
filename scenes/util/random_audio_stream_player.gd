...
@export var number_to_play = 2
@export var enable_pitch_randomization = false
@export var min_pitch_scale = 0.5
@export var max_pitch_scale = 1.5
@export var allow_sound_overlaping = false
...
func play():
	(
		...
		. new(get_children())
		. where(
			func(stream_player): return (
				allow_sound_overlaping
				or "playing" not in stream_player
				or not stream_player.playing
			)
		)
		. shuffled()
		. take(number_to_play)
