...
func follow_player(delta):
	...
	global_position = lerp(player_position, global_position, pow(2, lerp_power * delta))
...
func shake(delta):
	if shake_percentage > 0:
		x_noise_sample_position += x_noise_sample_direction * noise_sample_travel_rate * delta
		y_noise_sample_position += y_noise_sample_direction * noise_sample_travel_rate * delta
		...
		offset = Vector2(x_sample, y_sample) * max_shake_ofset * pow(shake_percentage, 2)
